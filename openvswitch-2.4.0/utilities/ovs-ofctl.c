...
/*
static void
ofctl_lock_switch(int argc, char *argv[])
{
    printf("lock switch \n");
    ...
    struct vconn *vconn;
    printf("1 \n");
    //rr->role = OFPCR12_ROLE_MASTER;
    printf("2 \n");
    protocol = open_vconn(argv[1], &vconn);
    //usable_protocols = OFPUTIL_P_ANY;
    //protocol = open_vconn_for_flow_mod(argv[1], &vconn, usable_protocols);
    printf("3 \n");
    //struct ofpbuf *msg = ofputil_encode_role_request(rr,protocol);
    send_openflow_buffer(vconn, ofputil_encode_role_request(rr,protocol));
    //transact_noreply(vconn, ofputil_encode_role_request(rr,protocol));
    printf("4 \n");
    free(rr);
    vconn_close(vconn);
}*/
...
static void
ofctl_flow_mod_2(int argc, char *argv[],char *cat, uint16_t command)
{
    if (argc > 2 && !strcmp(cat, "-")) {
        ofctl_flow_mod_file(argc, argv, command);
    } else {
        struct ofputil_flow_mod fm;
        char *error;
        ...
        error = parse_ofp_flow_mod_str(&fm, argc > 2 ? cat : "", command,
                                       &usable_protocols);
        if (error) {
            ovs_fatal(0, "%s", error);
        }
...
/* Contain locking information including lock flag and controller id*/
struct lock_info{
	bool lock;
	char *id;
};
...
ofctl_checklock_flows__(int argc, char *argv[], bool aggregate)
{
    ...
    int m_len = strlen("metadata=0x00000000/0xffffffff");
    lock_info.id =malloc(m_len+1);
    char *s[3];
    s[0] = argv[0];
    s[1] = argv[1];
    s[2] = "table=240";
...
static void
ofctl_unlock(struct ovs_cmdl_context *ctx)
{
	char *tmp = ctx->argv[2];
	//printf("%s\n", ctx->argv[2]);
	char *a = tmp;
	...
	strcat(id,a);
	strcat(id,"0000000/0xffff0000");
	struct lock_info lock_info_;
	lock_info_ = ofctl_checklock_flows__(ctx->argc, ctx->argv, false);

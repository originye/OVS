...

/* Encodes "role request" message 'request' for sending in the given
 * 'protocol'.  Returns the role request message, if 'protocol' supports them,
 * otherwise a null pointer. */
struct ofpbuf *
ofputil_encode_role_request(const struct ofputil_role_request *request,
                           enum ofputil_protocol protocol)
{
    enum ofp_version version;
    printf("in encode \n");
    printf("%d \n", OFP14_VERSION);
    //version = OFP14_VERSION;//ofputil_protocol_to_ofp_version(protocol);
    printf("%s \n", version);
    if (OFP14_VERSION >= OFP12_VERSION) {
        struct ofp12_role_request *rrequest;
        struct ofpbuf *buf;

        buf = ofpraw_alloc_xid(OFPRAW_OFPT12_ROLE_REQUEST, OFP12_VERSION, htonl(0),
                               0);
        rrequest = ofpbuf_put_zeros(buf, sizeof *rrequest);
        printf("encode role request 1 \n");
        rrequest->role = htonl(OFPCR12_ROLE_MASTER);
        printf("encode role request 2 \n");
        rrequest->generation_id = htonll(UINT64_MAX);
        printf("encode role request 3 \n");
        return buf;
    } else {
        return NULL;
    }
